...
export default function AdminProducts() {
  ...
  const recomputeTFIDF = async () => {
    ...
    try {
      ...
      darkSwal.fire({
        title: 'Terminé',
        text: 'Les tags IA ont été recalculés.',
        icon: 'success',
        timer: 2000,
      ...
      darkSwal.fire({
        title: 'Erreur',
        text: 'Impossible de recalculer les tags IA.',
        icon: 'error'
      });
    }
  };
  ...
  const openEdit = (p) => {
    ...
    setForm({ 
      ...
      stock: p.stock, category_id: p.category_id, is_active: p.is_active,
      rl_score: p.rl_score || 50.0 
    });
    setNewCatName("");
    setGalleryPreviews({ face: getFullImageUrl(p.image_url), dos: null, gauche: null, droite: null });
    setGalleryFiles({ face: null, dos: null, gauche: null, droite: null });
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (newCatName.trim() !== "") {
        const catRes = await axios.post(`${API_URL}/api/products/categories`, { name: newCatName });
      ...
      formData.append('rl_score', parseFloat(form.rl_score));
      ...
      // --- CORRECTION ICI : Ajout du config avec Content-Type: undefined ---
      const config = {
          headers: { 'Content-Type': undefined }
      };

      if (editProduct) { 
        await productsAPI.update(editProduct.id, formData, config); 
        toast.success("Système mis à jour");
      } else { 
        await productsAPI.create(formData, config); 
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try { await productsAPI.delete(id); load(); toast.success("Entrée supprimée"); } 
    ...
    <div className="max-w-7xl mx-auto px-4 py-10 min-h-screen text-white font-sans bg-[#020617]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        ...
        <div className="flex flex-1 max-w-2xl gap-3">
          ...
          <button onClick={recomputeTFIDF} className="px-4 py-3 bg-blue-600 hover:bg-blue-500 rounded-2xl font-black text-sm transition-all shadow-lg shadow-blue-900/20">
            ↻ Recalcul IA
          </button>
          <button onClick={openCreate} className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-2xl font-black text-sm transition-all shadow-lg shadow-emerald-900/20">
            + AJOUTER
          </button>
        </div>
      </div>
              ...
              {filteredProducts.map(p => (
                <tr key={p.id} className="group hover:bg-white/[0.01] transition-colors">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      <img src={getFullImageUrl(p.image_url)} alt="" className="w-12 h-12 rounded-xl object-cover border border-gray-800 group-hover:border-emerald-500/30 transition-all" />
                      <span className="font-black text-white text-base">{p.name}</span>
                    </div>
                  </td>
                  <td className="px-6 py-6 text-center">
                    <span className="font-mono text-emerald-400 font-bold">{parseFloat(p.rl_score || 0).toFixed(1)}%</span>
                  </td>
                  <td className="px-6 py-6 text-center">
                    <div className="flex flex-wrap justify-center gap-1">
                      {p.tags && p.tags.length > 0 ? (
                        p.tags.map((t, i) => (
                          <span key={i} className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-tighter">
                            #{t}
                          </span>
                        ))
                      ) : (
          ...
          <div className="bg-[#0b1120] border border-gray-800 rounded-[3rem] p-10 max-w-lg w-full shadow-2xl overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                {['face', 'dos', 'gauche', 'droite'].map((angle) => (
                  ...
                  >
                    {galleryPreviews[angle] ? (
                      <img src={galleryPreviews[angle]} className="w-full h-full object-cover" alt={angle} />
                    ) : (
              ...
              <div className="grid grid-cols-2 gap-4">
                <input type="number" placeholder="PRIX (AR)" required value={form.price} onChange={e => setForm({...form, price: e.target.value})} className="bg-[#0f172a] border border-gray-800 rounded-2xl px-6 py-4 outline-none focus:border-emerald-500/50" />
                <input type="number" placeholder="STOCK" required value={form.stock} onChange={e => setForm({...form, stock: e.target.value})} className="bg-[#0f172a] border border-gray-800 rounded-2xl px-6 py-4 outline-none focus:border-emerald-500/50" />
              </div>

              <div className="p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-2xl">
                <label className="text-[9px] font-bold text-emerald-500 uppercase tracking-widest mb-2 block">Intelligence RL (Score %)</label>
                <input type="number" step="0.1" value={form.rl_score} onChange={e => setForm({...form, rl_score: e.target.value})} className="w-full bg-transparent text-xl font-mono text-emerald-400 outline-none" />
              </div>
              ...
              <div className="space-y-3">
                ...
                <input 
                  type="text" placeholder="OU CRÉER NOUVEAU..." 
                  value={newCatName} onChange={(e) => { setNewCatName(e.target.value); setForm({...form, category_id: ""}); }}
              ...
              <textarea placeholder="DESCRIPTION POUR ANALYSE IA" rows={2} value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full bg-[#0f172a] border border-gray-800 rounded-2xl px-6 py-4 outline-none resize-none focus:border-emerald-500/50" />
              
              <div className="flex gap-4 mt-8">
                <button type="button" onClick={() => setShowForm(false)} className="flex-1 font-black text-gray-500 uppercase text-[10px] tracking-widest">Annuler</button>
                <button type="submit" className="flex-1 bg-emerald-600 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-500 shadow-lg shadow-emerald-900/20">Valider</button>
              </div>
            </form>
          </div>
        </div>
